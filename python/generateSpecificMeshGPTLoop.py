...
def runIgbextractVoltage(igbFile, dir=""):
    print(); print();
...
def run(args, job):
    ...
    files=["LQT2-His-1000-GM-0.30-S1S2-285-Stim-50-Rpmj-45e3-mGNa-0.7-pGNa-0.95-vtx-LeftMultiple_two-savF-1-Mlump-1-fit-1.25"
          ]
    
    Times=[ [286, 414, 480, 732] 
    ...
    for file in files:     
        ...
        #igbFileName=parentFolder + "/" + file + "/" + file + ".igb"
        igbFileName = os.path.join(parentFolder,file, file + ".igb")
        
        dirExtract=os.path.join(parentFolder,file); # Directory to extract AP file
        fullFileExtension = os.path.join(dirExtract,'apd.dat'); 
        if not os.path.isfile(fullFileExtension):   
            runIgbextractVoltage(igbFileName, dirExtract)
        
        
        # Read ElemenFile(colums 2 - 5 of .elem file)
        #dotElemFile = os.path.join(os.getcwd(), 'myopurk.elem') 
        dotElemFile = os.path.join(parentFolder,file,'myopurk.elem')    
        ...
        apdData = np.loadtxt(os.path.join(dirExtract,'apd.dat'))
        timeAxis=apdData[:,0]
        ...
        for i in range(lenT1):
            ...
            elemFile = file + "_" + str(args.startTime)
